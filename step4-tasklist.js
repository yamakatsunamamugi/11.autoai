...
(function () {
        ...
        if (trimmedValue === "プロンプト") {
          ...
          for (let i = 2; i <= 5; i++) {
            ...
            if (nextIndex < maxLength) {
              ...
              if (
                nextValue &&
                nextValue.toString().trim() === `プロンプト${i}`
              ) {
            ...
            if (
              ...
              console.log(
                `[step4-tasklist] 🖼️ DEBUG: WindowInfo取得 (aiType: ${aiType})`,
                {
                  ...
                  openedWindowsSize:
                    window.windowController?.openedWindows?.size,
              ...
              console.log(
                `[step4-tasklist] 🖼️ DEBUG: WindowController利用不可`,
                {
    ...
    async openWindows(windowLayout) {
      ...
      for (const layout of windowLayout) {
        try {
          ...
          ExecuteLogger.info(
            `🖼️ [WindowController] DEBUG: ウィンドウ作成結果`,
            {
              aiType: layout.aiType,
              position: layout.position,
    ...
    async function executeNormalAITask(task) {
      ...
      const executeFunction = async () => {
        switch (aiType) {
          case "chatgpt":
            ExecuteLogger.info(
              `[step4-execute.js] Step 4-6-8-5-1: ChatGPT実行`,
            );
  ...
  if (typeof window !== "undefined") {
    ...
    // WindowControllerクラスをエクスポート（step0で既にインスタンスが作成されている場合は上書きしない）
    if (!window.WindowController) {
      window.WindowController = WindowController;
    }

    // windowControllerインスタンスが存在しない場合は作成
    if (!window.windowController) {
      window.windowController = new WindowController();
      window.windowController.initializeWindowService();
      ExecuteLogger.info("✅ WindowController インスタンス作成・初期化");
    }
