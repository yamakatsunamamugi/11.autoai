...
if (typeof chrome !== "undefined" && chrome.storage && chrome.storage.local) {
  chrome.storage.local.get("logLevel", (result) => {
    if (result.logLevel) {
      ...
      console.info(
        `📋 [ChatGPT Automation] ログレベル設定: ${["", "ERROR", "WARN", "INFO", "DEBUG"][CURRENT_LOG_LEVEL]}`,
      );
    }
  });
}
  ...
  try {
    executeTask = async function executeTaskImpl(taskData) {
      ...
      try {
        ...
        if (featureName && featureMapping[featureName]) {
          logWithTimestamp(
        ...
        // テキスト取得前のDOM安定性チェック
        logWithTimestamp("【Step 4-7】テキスト取得前の安定性チェック", "info");
        await sleep(3000);
