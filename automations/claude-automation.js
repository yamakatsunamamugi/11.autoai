...
(async function() {
    ...
    const recreateWindow = async () => {
        console.log('【Claude-ウィンドウ再作成】🔄 処理開始...');

        try {
            // 現在のウィンドウを閉じる
            console.log('【Claude-ウィンドウ再作成】ステップ1: タブリロード実行中...');
            await chrome.tabs.reload();
            console.log('【Claude-ウィンドウ再作成】ステップ2: 安定化待機中（2秒）...');
            await wait(2000);

            // ページを再読み込みしてセレクタを再ロード
            console.log('【Claude-ウィンドウ再作成】ステップ3: セレクタ再読み込み中...');
            await loadSelectors();

            console.log('【Claude-ウィンドウ再作成】✅ 全工程完了');
            return true;
        } catch (error) {
            console.error('【Claude-ウィンドウ再作成】❌ エラー発生:', error);
            return false;
        }
    };
