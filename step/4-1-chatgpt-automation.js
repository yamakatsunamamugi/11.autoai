...
(async function() {
    ...
    class ChatGPTRetryManager {
        ...
        classifyError(error, context = {}) {
            ...
            if (errorMessage.includes('rate limit') ||
                ...
                console.log(`⚠️ [Step 4-1-0-3] レート制限エラー検出:`, {
                    ...
                    reason: 'ChatGPT APIレート制限により即座に新規ウィンドウでリトライが必要'
                });
                return errorType;
            }
        ...
        async executeWithEscalation(config) {
            ...
            while (retryCount < 20) { // 最大20回
                try {
                    ...
                    console.log(`🔄 [Step 4-1-0-3] ${actionName} 試行 ${retryCount}/20:`, {
                    ...
                    if (isSuccess(lastResult)) {
                        ...
                        console.log(`✅ [Step 4-1-0-3] ${actionName} 成功（${retryCount}回目）:`, {
                            attemptNumber: retryCount,
                            totalTime,
                            successRate: (this.metrics.successfulAttempts / this.metrics.totalAttempts * 100).toFixed(2) + '%',
                            result: lastResult
                        });
                    ...
                    console.error(`❌ [Step 4-1-0-3] ${actionName} エラー (${retryCount}回目):`, {
                        errorType,
                    ...
                    if (escalationResult && escalationResult.success) {
                        console.log(`✅ [Step 4-1-0-3] エスカレーション成功:`, escalationResult);
            ...
            // 全リトライ失敗
            const totalTime = Date.now() - startTime;
            const finalErrorType = lastError ? this.classifyError(lastError, context) : 'UNKNOWN';
        ...
        async executeEscalation(level, context) {
            ...
            console.log(`🔄 [ChatGPT-Escalation] ${level} 実行開始 (${retryCount}回目)`);
        ...
        async waitWithEscalationStrategy(level, retryCount, errorType) {
            ...
            if (delay > 0) {
                ...
                console.log(`⏳ [ChatGPT-Wait] ${level} - ${delayMinutes}分後にリトライします...`);
    ...
    const loadSelectors = async () => {
        ...
        try {
            ...
            log('【Step 4-1-0-2】❌ Failed to load ui-selectors-data.json: ' + error.message, 'error');
    ...
    async function standardWaitForResponse() {
        ...
        if (stopBtn) {
            log('【Step 4-1-6-2】停止ボタンが10秒間消えるまで待機（最大5分）', 'info');
            ...
            while (disappearWaitCount < 300) {
                ...
                if (!stopBtn) {
                    confirmCount++;
                    if (confirmCount >= 10) {
                        log('【Step 4-1-6-2】✅ 応答完了（停止ボタンが10秒間非表示）', 'success');
                        break;
                    }
                } else {
                    confirmCount = 0;
    ...
    async function retryWithPrompt() {
        log('【Step 4-1-6-3】再送信処理（「いいから元のプロンプトを確認して作業をして」）', 'step');
        ...
        for (let i = 0; i < maxWaitTime; i++) {
            ...
            if (!stopBtn) {
                ...
                if (consecutiveAbsent >= 10) {
                    log('【Step 4-1-6-3】停止ボタンが10秒間連続で消滅。完了！', 'success');
                    break;
                }
            } else {
                consecutiveAbsent = 0;
    ...
    async function executeTask(taskData) {
        ...
        try {
            ...
            if (modelName) {
                ...
                log('【Step 4-1-3-1】モデルメニューを開いて利用可能なモデルを取得', 'step');
            ...
            while (!sendSuccess && sendAttempts < maxSendAttempts) {
                sendAttempts++;
                log(`【Step 4-1-5-${sendAttempts}】送信試行 ${sendAttempts}/${maxSendAttempts}`, 'step');
            ...
            if (!responseText) {
                ...
                if (assistantMessages.length > 0) {
                    ...
                    if (normalElement) {
                        log('🚫 【Step 4-1-7-3】プロンプト除外機能を適用してテキスト取得（通常応答）', 'info');
                        responseText = normalElement.textContent?.trim() || '';
                        if (responseText.length > 10) {
                            log('✅ 【Step 4-1-7-4】プロンプト除外完了 - 純粋なAI応答を取得', 'success');
                    ...
                    // 上記で取得できない場合のフォールバック
                    if (!responseText) {
                        log('🚫 【Step 4-1-7-1】プロンプト除外機能を適用してテキスト取得', 'info');
                        const text = getCleanText(lastMessage);
                        if (text && text.length > 10) {
                            responseText = text;
                            log('✅ 【Step 4-1-7-2】プロンプト除外完了 - 純粋なAI応答を取得', 'success');
