...
function checkNextGroup() {
  ...
  console.log('[step6-nextgroup.js] [Step 6-1-2] 次のグループなし（全グループ処理完了）', {
    ...
    総グループ数: totalGroups,
    完了率: totalGroups > 0 ? Math.round((processedGroups.length / totalGroups) * 100) + '%' : '0%'
  });
  return null;
}
  ...
  try {
    ...
    if (window.globalState) {
      ...
      if (window.globalState.stats) {
        window.globalState.stats.completedGroups = (window.globalState.stats.completedGroups || 0) + 1;
      }
    }

    console.log(`[step6-nextgroup.js] [Step 6-2-1] グループ番号をインクリメント: ${prevIndex + 1} → ${(window.globalState?.currentGroupIndex || 0) + 1}`, {
      前のインデックス: prevIndex,
    ...
    console.log(`[step6-nextgroup.js] [Step 6-2-1] グループ ${(window.globalState?.currentGroupIndex || 0) + 1} 処理開始:`, {
...
function calculateStatistics() {
  ...
  console.log(`[step6-nextgroup.js] [Step 6-3-2] 成功タスク数: ${successTasks}件`);
  console.log(`[step6-nextgroup.js] [Step 6-3-2] 失敗タスク数: ${failedTasks}件`);
  ...
  const statistics = {
    ...
    成功率: totalTasks > 0 ? Math.round((successTasks / totalTasks) * 100) + '%' : '0%'
  };
  ...
  if (failedTasks > 0) {
    console.warn('[step6-nextgroup.js] [Step 6-3-2] 警告: 失敗タスクあり', {
      失敗率: totalTasks > 0 ? Math.round((failedTasks / totalTasks) * 100) + '%' : '0%',
...
`;
  ...
  if (processedGroups.length > 0) {
    ...
    processedGroups.forEach((item, index) => {
      if (item && item.group) {
        console.log(`   ${index + 1}. ${item.group.taskType || item.group.type || '不明'} - ${item.timestamp || '不明'}`, {
...
async function executeStep6(taskGroups = [], currentIndex = 0) {
  ...
  console.log('[Step 6] 入力パラメータ:', {
    ...
    グループ詳細: (Array.isArray(taskGroups) && taskGroups.length > 0) ? taskGroups.slice(0, 3) : []
  });
  ...
  try {
    ...
    console.error('[step6-nextgroup.js] [Step 6] エラー発生:', {
      ...
      現在の状態: {
        ...
        processedGroupsExists: Array.isArray(window.globalState?.processedGroups)
      }
    });
...
console.log('[step6-nextgroup.js] ✅ Step6関数定義完了（復元版 + 防御的プログラミング適用）');