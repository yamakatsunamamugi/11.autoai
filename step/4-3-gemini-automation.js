...
(async function() {
    ...
    class GeminiRetryManager {
        ...
        async executeWithEscalation(config) {
            ...
            while (retryCount < 20) { // 最大20回
                try {
                    ...
                    console.log(`🔄 [Step 4-3-Retry] ${actionName} 試行 ${retryCount}/20`);
                    ...
                    if (isSuccess(lastResult)) {
                        ...
                        console.log(`✅ [Step 4-3-Retry] ${actionName} 成功（${retryCount}回目）`);
                        return {
                            success: true,
                            result: lastResult,
                            retryCount,
                    ...
                    console.error(`❌ [Step 4-3-Retry] ${actionName} エラー (${retryCount}回目):`, {
                        errorType,
        ...
        async executeEscalation(level, context) {
            ...
            console.log(`🔄 [Step 4-3-Escalation] ${level} 実行開始 (${retryCount}回目)`);
        ...
        async waitWithEscalationStrategy(level, retryCount, errorType) {
            ...
            if (delay > 0) {
                ...
                console.log(`⏳ [Step 4-3-Wait] ${level} - ${delayMinutes}分後にリトライします...`);
    ...
    const loadSelectors = async () => {
        ...
        try {
            ...
            log('【Step 4-3-0-2】❌ Failed to load ui-selectors-data.json: ' + error.message, 'error');
    ...
    const removePromptFromText = (fullText, sentPrompt = null) => {
        ...
        try {
            ...
            if (patternFound) {
                log('【Step 4-3-除外】パターンマッチングでプロンプトを除外しました', 'success');
            }
    ...
    async function discoverModelsAndFeatures() {
        ...
        try {
            ...
            if (menuButton) {
                ...
                if (menuContainer) {
                    ...
                    log(`【Step 4-3-1-1】モデル探索完了: ${window.availableModels.length}個のモデルを発見`, 'success');
                }
            }
        } catch (e) {
        ...
        try {
            ...
            log('【Step 4-3-1-2】機能探索完了: ${window.availableFeatures.length}個の機能を発見', 'success');
    ...
    async function executeCore(modelName, featureName, promptText) {
        ...
        try {
                        ...
                        if (modelButtonToClick) {
                            ...
                            if (currentModelDisplay) {
                                ...
                                if (displayText.includes(normalizedModelName)) {
                                    log(`【Step 4-3-2-3】✅ モデル選択確認成功: 「${displayText}」が選択されています`, 'success');
                                } else {
                                    log(`【Step 4-3-2-3】⚠️ モデル表示が期待値と異なります。期待値: ${modelName}, 実際: ${displayText}`, 'warn');
                                }
                            }
                        } else {
                            log(`【Step 4-3-2-3】モデル "${modelName}" が見つからないため、デフォルトを使用`, 'warn');
                        }
                    }
                }
            ...
            await logStep('【Step 4-3-3】機能選択', async () => {
                log(`【Step 4-3-3-1】選択する機能: '${featureName || '設定なし'}'`, 'info');
                ...
                if (featureName && featureName !== 'none' && featureName !== '通常') {
                    ...
                    log(`【Step 4-3-3-2】🔍 メインボタン数: ${allButtons.length}`, 'info');
                    ...
                    if (featureButton) {
                        ...
                        log(`【Step 4-3-3-3】✅ 機能「${featureName}」を選択しました`, 'success');
                        ...
                        if (selectedButton) {
                            ...
                            if (selectedText.toLowerCase() === featureName.toLowerCase() ||
                                ...
                                log(`【Step 4-3-3-3】✅ 機能選択確認成功: 「${selectedText}」が有効化されています`, 'success');
                            } else {
                                log(`【Step 4-3-3-3】⚠️ 機能選択確認: 期待された機能「${featureName}」と異なる機能「${selectedText}」が選択されています`, 'warn');
                            }
                        } else {
                            log(`【Step 4-3-3-3】⚠️ 機能の選択状態が確認できません`, 'warn');
                        }
                    } else {
                        log(`【Step 4-3-3-3】機能 "${featureName}" が見つからないため、スキップ`, 'warn');
                    }
                }
            ...
            await logStep('【Step 4-3-5】メッセージ送信（再試行対応）', async () => {
                ...
                while (!sendSuccess && sendAttempts < maxSendAttempts) {
                    sendAttempts++;
                    log(`【Step 4-3-5-${sendAttempts}】送信試行 ${sendAttempts}/${maxSendAttempts}`, 'step');
                    ...
                    if (!sendButton) {
                        ...
                        log(`【Step 4-3-5-${sendAttempts}】送信ボタンが見つかりません。2秒後に再試行...`, 'warning');
                        await wait(2000);
                        continue;
                    }
                    
                    sendButton.click();
                    log(`【Step 4-3-5-${sendAttempts}】送信ボタンをクリックしました（試行${sendAttempts}）`, 'success');
                    await sleep(1000);
                    ...
                    for (let i = 0; i < 5; i++) {
                        ...
                        if (stopButton) {
                            ...
                            log(`【Step 4-3-5-${sendAttempts}】停止ボタンが表示されました - 送信成功`, 'success');
                            break;
                        }
                        await sleep(1000);
                    }
                    ...
                    if (stopButtonAppeared) {
                        ...
                        log(`【Step 4-3-5-${sendAttempts}】送信反応が確認できません。再試行します...`, 'warning');
                        await wait(2000);
                    }
                }
                ...
                log(`【Step 4-3-5-記録】🔍 送信時刻記録開始 - AIHandler: ${!!window.AIHandler}, recordSendTimestamp: ${!!window.AIHandler?.recordSendTimestamp}, currentAITaskInfo: ${!!window.currentAITaskInfo}`, 'info');
                if (window.AIHandler && window.AIHandler.recordSendTimestamp) {
                    try {
                        log(`【Step 4-3-5-記録】📝 送信時刻記録実行開始 - タスクID: ${window.currentAITaskInfo?.taskId}`, 'info');
                        await window.AIHandler.recordSendTimestamp('Gemini');
                        log(`【Step 4-3-5-記録】✅ 送信時刻記録成功`, 'success');
                    } catch (error) {
                        log(`【Step 4-3-5-記録】❌ 送信時刻記録エラー: ${error.message}`, 'error');
                    }
                } else {
                    log(`【Step 4-3-5-記録】⚠️ AIHandler または recordSendTimestamp が利用できません`, 'warning');
                }
            ...
            // ステップ4: 応答待機
            // ========================================
            const responseText = await logStep('【Step 4-3-4】応答待機', () => new Promise(async (resolve, reject) => {
                ...
                log(`【Step 4-3-4-0】待機モード: ${isDeepResearchMode ? '🔬 Deep Research' : isCanvasMode ? '🎨 Canvas' : '💬 通常'}`, 'info');
                ...
                if (isDeepResearchMode) {
                    ...
                    try {
                        ...
                        logDr('【Step 4-3-4-2】「リサーチを開始」ボタンをクリックしました。', 'success');
                        await wait(2000);
                        ...
                        const loggingInterval = setInterval(() => {
                            ...
                            logDr(`【Step 4-3-4-3】[定期チェック] 回答停止ボタンは${btn ? '✅ 存在します' : '❌ 存在しません'}。`);
                        }, 10000);
                        ...
                        const checkInterval = setInterval(() => {
                            if (findElement(['button.send-button.stop'])) {
                                ...
                                logDr('【Step 4-3-4-3完了】Deep Researchの応答が完了しました。', 'success');
                    ...
                    log('【Step 4-3-4-2】Canvasモード: テキスト生成の監視を開始します。');
                    ...
                    const monitor = setInterval(() => {
                        ...
                        if (Date.now() - lastChangeTime > 10000) {
                            ...
                            log('【Step 4-3-4-2】10秒間テキストの更新がなかったため、処理を完了します。', 'success');
                    ...
                    const checker = setInterval(() => {
                        ...
                        log(`【Step 4-3-4-2】[待機中] 応答生成を待っています... (${waitTime / 1000}秒)`);
                        waitTime += 2000;
                    }, 2000);
                }
            }));
            ...
            await logStep('【Step 4-3-7】テキスト取得', async () => {
                ...
                for (const selector of canvasSelectors) {
                    ...
                    if (canvasElement) {
                        log('🚫 【Step 4-3-7-1】プロンプト除外機能を適用してテキスト取得（Canvas応答）', 'info');
                        text = getStructuredCanvasContent(canvasElement);
                        if (text && text.length > 10) {
                            log('✅ 【Step 4-3-7-2】プロンプト除外完了 - 純粋なAI応答を取得', 'success');
                            log(`【Step 4-3-7-1】Canvas/拡張応答取得成功 (${selector}): ${text.length}文字`, 'success');
                            break;
                        }
                    }
                }
                ...
                if (!text) {
                    ...
                    for (const selector of normalSelectors) {
                        ...
                        if (responseElements.length > 0) {
                            ...
                            log('🚫 【Step 4-3-7-3】プロンプト除外機能を適用してテキスト取得（通常応答）', 'info');
                            text = getCleanText(latestResponse);

                            if (text && text.length > 10) {
                                log('✅ 【Step 4-3-7-4】プロンプト除外完了 - 純粋なAI応答を取得', 'success');
                                log(`【Step 4-3-7-2】通常テキスト取得成功 (${selector}): ${text.length}文字`, 'success');
                                break;
                            }
                        }
                    }
                ...
                if (!text) {
                    ...
                    for (const selector of fallbackSelectors) {
                        ...
                        if (elements.length > 0) {
                            ...
                            log('🚫 【Step 4-3-7-5】プロンプト除外機能を適用してテキスト取得（フォールバック）', 'info');
                            text = getCleanText(lastElement);
                            if (text && text.length > 10) {
                                log('✅ 【Step 4-3-7-6】プロンプト除外完了 - 純粋なAI応答を取得', 'success');
                                log(`【Step 4-3-7-3】フォールバック取得成功 (${selector}): ${text.length}文字`, 'success');
                                break;
                            }
                        }
                    }
                ...
                log(`【Step 4-3-7-完了】最初の100文字: ${text.substring(0, 100)}...`, 'info');
            ...
            try {
                ...
                if (window.ModelInfoExtractor) {
                    ...
                    log(`【Step 4-3-確認-1】📊 選択後確認 - 実際のモデル: "${displayedModel}"`, 'info');
                } else {
                    log('【Step 4-3-確認-1】⚠️ ModelInfoExtractorが利用できません', 'warn');
                }

                // FunctionInfoExtractorを使用
                if (window.FunctionInfoExtractor) {
                    displayedFunction = window.FunctionInfoExtractor.extract('Gemini') || '';
                    log(`【Step 4-3-確認-2】📊 選択後確認 - 実際の機能: "${displayedFunction}"`, 'info');
                } else {
                    log('【Step 4-3-確認-2】⚠️ FunctionInfoExtractorが利用できません', 'warn');
                }
            } catch (error) {
                log(`【Step 4-3-確認】⚠️ モデル/機能情報取得エラー: ${error.message}`, 'warn');
            }
