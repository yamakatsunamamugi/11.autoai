...
export function setupMessageHandler() {
  ...
  chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    switch (request.action || request.type) {
      ...
      // ===== 送信時刻記録処理 =====
      case "RECORD_SEND_TIMESTAMP":
        console.log('[MessageHandler] 送信時刻記録要求:', request.data);
        (async () => {
          try {
            const { taskId, aiType, model, function: functionName, timestamp } = request.data;

            // SheetsClientインスタンスを取得または作成
            if (!globalThis.sheetsClient) {
              const SheetsClient = await import('../features/spreadsheet/sheets-client.js');
              globalThis.sheetsClient = new SheetsClient.default();
            }

            // 送信時刻を記録
            if (globalThis.sheetsClient && typeof globalThis.sheetsClient.recordSendTimestamp === 'function') {
              globalThis.sheetsClient.recordSendTimestamp(taskId, {
                aiType: aiType,
                model: model,
                function: functionName || '通常',
                timestamp: timestamp ? new Date(timestamp) : new Date()
              });

              console.log(`✅ [MessageHandler] 送信時刻記録成功: taskId=${taskId}`);
              sendResponse({ success: true });
            } else {
              console.warn('[MessageHandler] SheetsClient.recordSendTimestampが利用できません');
              sendResponse({ success: false, error: 'recordSendTimestamp not available' });
            }
          } catch (error) {
        ...
        (async () => {
          try {
            ...
            console.log('[Claude-Direct] ログ書き込み開始:', {
              ...
              spreadsheetId: taskData.spreadsheetId,  // デバッグ用: spreadsheetId確認
              gid: taskData.gid,  // デバッグ用: gid確認
