  ...
  while (retryCount < maxRetries) {
    try {
      if (retryCount === 1 || retryCount === maxRetries - 1) {
        console.log(
          `[WindowService] ${actionName} 再試行 ${retryCount}/${maxRetries}`,
          context,
        );
      }
      ...
      if (isSuccess(lastResult)) {
        if (retryCount > 0) {
          console.log(
            `[WindowService] ✅ ${actionName} 成功（${retryCount}回目の試行）`,
            context,
          );
        }
...
export class WindowService {
  ...
  static initializeWindowMonitoring() {
    ...
    if (
      ...
      console.warn(
        "⚠️ [WindowService] chrome.windows.onRemoved が利用できません",
      );
    }
  }
  ...
  static handleUnexpectedWindowClosure(windowId) {
    ...
    if (windowInfo) {
      ...
      if (windowInfo.enableAutoRecovery !== false) {
        setTimeout(async () => {
          try {
            await this.attemptWindowRecovery(
              windowId,
              windowInfo,
              windowInfo.currentTaskId,
            );
          } catch (recoveryError) {
            console.error(
              "🔄 [WindowService] 自動復旧処理エラー:",
              recoveryError,
            );
          }
        }, 2000); // 2秒後に復旧を試行
  ...
  static cleanupClosedWindow(windowId) {
    ...
    if (position !== undefined) {
      ...
      console.log(
        `🧹 [WindowService] ポジション${position}をクリーンアップしました`,
      );
    }
    ...
    try {
      ...
      if (typeof globalThis !== "undefined" && globalThis.logManager) {
        await globalThis.logManager.logError(
          `ウィンドウ異常終了検出: ${windowInfo.aiType || "不明"} (ID: ${windowId})`,
        );
      }
    } catch (error) {
      console.error(
        "📝 [WindowService] スプレッドシートログ記録エラー:",
        error,
      );
    }
  }
  ...
  static async attemptWindowRecovery(windowId, windowInfo, taskId = null) {
    ...
    try {
      ...
      if (windowInfo.aiType && this.AI_URLS[windowInfo.aiType]) {
        const newWindowInfo = await this.openAIWindow(
          windowInfo.aiType,
          originalPosition,
        );
  ...
  static async createAIWindow(url, options = {}) {
    ...
    try {
      ...
      const totalTime = (performance.now() - startTime).toFixed(0);
      console.error(
        `[WindowService] AIウィンドウ作成エラー (${totalTime}ms):`,
        error,
      );
      throw error;
    }
    ...
    if (monitorNumber > 0 && monitorNumber <= displays.length) {
      ...
      return {
        left:
          ...
          Math.floor((display.workArea.width - width) / 2),
        top:
          display.workArea.top +
          Math.floor((display.workArea.height - height) / 2),
        width: width,
        height: height,
      };
    }
  ...
  static async openAIWindow(aiType, position = null, options = {}) {
    console.log(
      `[WindowService] AIウィンドウを開く: ${aiType}, ポジション: ${position}`,
    );
  ...
  static async closeWindow(
    ...
    try {
      ...
      console.log(
        `✅ [WindowService] ウィンドウ削除完了: ${windowId} (${elapsed}ms)`,
        {
          reason,
          source,
          elapsed,
      ...
      if (
        ...
        console.warn(`⚠️ [WindowService] ウィンドウは既に閉じられています:`, {
          ...
          elapsed,
          message:
            "ウィンドウが予期せず閉鎖済み（ユーザー操作またはクラッシュの可能性）",
        });
      } else {
      ...
      if (onClosed && typeof onClosed === "function") {
        try {
          ...
          console.error(
            "[WindowService] ウィンドウ閉じ後コールバックエラー:",
            callbackError,
          );
        }
      }
  ...
  static async closeAllWindows(reason = "一括閉鎖") {
    ...
    for (const [windowId] of this.activeWindows) {
      closePromises.push(
        this.closeWindow(windowId, null, reason, "closeAllWindows"),
      );
    }
  ...
  static async createWindowWithPosition(url, position, options = {}) {
    ...
    if (this.windowPositions.has(position)) {
      ...
      console.warn(
        `[WindowService] ポジション${position}は既に使用中: Window${existingWindowId}`,
      );
      ...
      try {
        ...
        await this.closeWindow(
          ...
          null,
          "既存ウィンドウの置き換え",
          "WindowService.createWindowWithPosition",
        );
        ...
        while (this.windowPositions.has(position) && retryCount < maxRetries) {
        ...
        if (this.windowPositions.has(position)) {
          console.warn(
            `[WindowService] ポジション${position}の削除確認に失敗、強制的にクリア`,
          );
          this.windowPositions.delete(position);
        }

        console.log(
          `[WindowService] ポジション${position}の削除完了確認、新規作成開始`,
        );
      } catch (closeError) {
        console.error(
          `[WindowService] 既存ウィンドウの閉じ処理でエラー:`,
          closeError,
        );
        // エラーでも強制的にポジションをクリア
        this.windowPositions.delete(position);
    ...
    try {
      ...
      if (
        needsPositionUpdate &&
        positionInfo.left !== undefined &&
        positionInfo.top !== undefined
      ) {
        const updateStartTime = performance.now();
        console.log(
          `[WindowService] ウィンドウ位置補正が必要: 期待=${positionInfo.left},${positionInfo.top} 実際=${createdWindow.left},${createdWindow.top}`,
        );
        ...
        try {
          ...
          console.log(
            `[WindowService] ウィンドウ位置補正完了 (${updateTime}ms)`,
          );
        } catch (updateError) {
          console.warn(
            "[WindowService] ウィンドウ位置の更新に失敗:",
            updateError,
          );
        }
      }
        ...
        await executeSimpleRetry({
          action: async () => {
            ...
            if (tab.status === "complete") {
              console.log(
                `[WindowService] ポジション${position}のタブ読み込み完了`,
              );
              return true;
            }
            return null;
  ...
  static debug() {
    ...
    console.log(
      "  使用中ポジション:",
      Array.from(this.windowPositions.keys()).sort(),
    );
    console.log("  ウィンドウ一覧:");
    for (const [id, info] of this.activeWindows) {
      const position = this.positionToWindow.get(id);
      console.log(
        `    - ID: ${id}, Position: ${position ?? "なし"}, Type: ${info.type}, URL: ${info.url}`,
      );
    }
  }
  ...
  static async openAllAISites() {
    ...
    try {
      ...
      console.log(
        `[WindowService] 既に開かれているAIサイト: ${openAISites.length}/3`,
      );
      console.log(
        `[WindowService] 開かれているサイト: ${openAISites.map((s) => s.name).join(", ") || "なし"}`,
      );

      // Step 4: 未開放サイトを4分割レイアウトで作成
      const createdWindows = [];
      const unopenedSites = aiSites.filter(
        (site) => !openAISites.some((openSite) => openSite.name === site.name),
      );
      ...
      for (const site of unopenedSites) {
        try {
          console.log(
            `[WindowService] ${site.name}を位置${site.position}に作成中...`,
          );

          // WindowServiceの既存機能を使用
          const window = await this.createWindowWithPosition(
            site.url,
            site.position,
            {
              type: "popup",
          ...
          console.log(
            `[WindowService] ✅ ${site.name}作成完了 (Window ${window.id})`,
          );
      ...
      const totalTime = (performance.now() - startTime).toFixed(0);
      console.log(
        `[WindowService] AIサイト一括オープン完了: ${createdWindows.length}個作成 (${totalTime}ms)`,
      );
      ...
      const totalTime = (performance.now() - startTime).toFixed(0);
      console.error(
        `[WindowService] AIサイト一括オープンエラー (${totalTime}ms):`,
        error,
      );
      return {
        success: false,
