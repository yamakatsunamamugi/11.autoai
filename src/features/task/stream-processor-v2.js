      ...
      for (const answerColIndex of answerCols) {
        ...
        if (!hasAnswer) {
          ...
          tasks.push({
            ...
            // スプレッドシート情報を追加
            spreadsheetId: this.spreadsheetData?.spreadsheetId || spreadsheetData?.spreadsheetId,
            gid: this.spreadsheetData?.gid || spreadsheetData?.gid,
    ...
    try {
      ...
      if (this.sheetsClient) {
        ...
        if (typeof this.sheetsClient.recordSendTimestamp === 'function') {
          this.sheetsClient.recordSendTimestamp(taskId, {
