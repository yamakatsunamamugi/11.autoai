...
class TaskGenerator {
  ...
  async generateAITasksForGroup(
    ...
    for (const workRow of workRows) {
      ...
      // プロンプト2〜5を順番に読み込み
      if (columnGroup.additionalPromptColumns && columnGroup.additionalPromptColumns.length > 0) {
        console.log(`[TaskGenerator] 追加プロンプト列を確認: ${columnGroup.additionalPromptColumns.join(', ')}`);
        
        for (const addPromptCol of columnGroup.additionalPromptColumns) {
          const additionalText = this.getCellValue(
            spreadsheetData,
            addPromptCol,
            workRow.number
          );
          ...
          if (additionalText && additionalText.trim()) {
            allPrompts.push(additionalText);
            additionalPromptsFound.push(addPromptCol);
            console.log(`[TaskGenerator] ${addPromptCol}列から追加プロンプト取得（${additionalText.length}文字）`);
          } else {
            console.log(`[TaskGenerator] ${addPromptCol}列は空`);
          }
        }
      }
      ...
      if (additionalPromptsFound.length > 0) {
        console.log(
          `[TaskGenerator] 行${workRow.number}: 追加プロンプトを${additionalPromptsFound.length}個検出: ${additionalPromptsFound.join(', ')}`
        );
      }

      // すべてのプロンプトを改行で連結
      const combinedPrompt = allPrompts.join('\n');
      console.log(`[TaskGenerator] 結合されたプロンプト: ${allPrompts.length}個のパーツ、合計${combinedPrompt.length}文字`);
