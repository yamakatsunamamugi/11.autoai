/**
 * ========================================
 * step3-tasklist.js - バッチタスク実行管理
 * ========================================
 *
 * 【実行フロー】
 *
 * [3-0] executeStep3() ← step2から開始
 *   ├─ タスクグループをバッチ処理（並列3タスク）
 *   ├─ 各タスク完了時に handleIndividualTaskCompletion() 実行
 *   └─ 全タスク完了後は何もしない（グループ完了チェックはhandleIndividualTaskCompletion内）
 *
 * [3-0] handleIndividualTaskCompletion()
 *   ├─ スプレッドシート記載
 *   ├─ ウィンドウクローズ
 *   ├─ DynamicTaskSearch.registerCompletion() - 完了登録
 *   └─ （Phase 4削除済み - 次タスク探索なし）
 *
 * [3-4] checkAndHandleGroupCompletion()
 *   ├─ window.checkCompletionStatus() ← step2の[2-2-2]を呼び出し
 *   ├─ グループ完了判定
 *   └─ 完了なら window.executeStep2() → step2の[2-1]に戻る
 *       └─ executeStep3()を再度呼び出してバッチ処理
 *
 * ========================================
 */
...
async function executeStep3(taskList) {
  ...
  try {
    ...
    if (processTaskList.length === 0) {
      ...
      ExecuteLogger.info(
        "📝 [Step 4-6-3] ウィンドウ事前作成をスキップ（動的作成モード）",
      );
    }
